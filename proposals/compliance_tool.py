...
class ComplianceTool:
    ...
    def upload_src(self, src, filename, bucketName):
        success = False
        ...
        try:
            head = self.s3_client.head_object(Bucket=bucketName, Key=filename)
        except ClientError:
            etag = ''
        else:
    ...
    def run(self, proposal):
        ...
        file_path = proposal.nofo
        toc_page = proposal.toc
        ...
        for i in title_list:
            ...
            in_mem_file.seek(0)
            print("sumpin")
            self.upload_src(in_mem_file, "media/" + title_name, os.environ['AWS_STORAGE_BUCKET_NAME'])
            print("more")
            ...
            in_mem_file.seek(0)
            self.upload_src(in_mem_file, "media/" + content_name, os.environ['AWS_STORAGE_BUCKET_NAME'])
